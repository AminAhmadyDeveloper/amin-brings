...
function Calendar({
  ...
  const _monthGridClassName = cn(
    'mx-auto mt-4 h-full w-full',
    props.monthGridClassName,
  );
  const _weekClassName = cn(
    'mt-2 flex w-full items-start justify-between',
    props.weekClassName,
  );
  ...
  const _weeksClassName = cn(
    'flex h-full flex-col justify-between pb-4',
    props.weeksClassName,
  );
    ...
    <DayPicker
      ...
      formatters={{
        formatWeekdayName: (date) => {
          return format(date, 'EEEE').slice(0, 2);
        },
        formatDay: (date) => {
...
function CaptionLabel({
  ...
  const yearFrom = format(new Date(`${displayYears.from}-01-01`), 'yyyy');

  const yearTo = format(new Date(`${displayYears.to}-01-01`), 'yyyy');
  ...
  return (
    ...
    >
      {navView === 'days' ? children : yearFrom + ' - ' + yearTo}
    </Button>
  );
}
    ...
    <div className={cn('grid grid-cols-4 gap-y-2', className)} {...props}>
      {Array.from(
        ...
        (_, i) => {
          ...
          const yearFrom = format(
            new Date(`${displayYears.from}-01-01`),
            'yyyy',
          );
